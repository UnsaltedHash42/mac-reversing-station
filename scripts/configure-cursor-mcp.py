#!/usr/bin/env python3
"""Write or update Cursor MCP entries for the Keep."""
...
def main(argv: list[str]) -> int:
    ...
    if current == rendered:
        print(f"OK - Cursor MCP config already up to date: {args.config}")
        return 0
    ...
    print(f"OK - wrote Cursor MCP config: {args.config}")
    print("Restart Cursor so the MCP server list refreshes.")
    return 0
