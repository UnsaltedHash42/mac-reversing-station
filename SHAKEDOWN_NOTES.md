...
| 8 | `-readOnly` was the default in ad-hoc scripts — analyzed Ghidra project discarded on exit. On a 177 MB binary this is 30-60 min of work thrown away per scan. | old one-off scripts | Fixed 2026-05-13. SSH path: `ghidra-scan.sh` already drops `-readOnly` (no change needed). MCP path: empirically verified against pinned `b9c491a` on NightBlood — `read_only=true` is **session-level mutability gate** (raises on `function.rename` etc.) but `update_analysis=true` runs analysis in a transaction that persists into the project DB regardless of `read_only`. Probe sequence (open `read_only=true update_analysis=true` → close → reopen `read_only=true update_analysis=false`) returned the analysis-applied function table from the first session, confirming persistence across reopen. SKILL.md Workflow A's documented `read_only=true, update_analysis=true` is correct as written. New "Mutability Semantics" subsection added to SKILL.md so future agents don't re-derive the gating rules. Also surfaced (not closed here): `program.save` ignores `read_only`; the MCP path silently picks one slice on universal Mach-Os (different problem class — captured for next session). | shipped |
...
| 22 | `mcp__ghidra-mcp__project_program_open_existing` requires `program_name` or `program_path` despite the tool's name + description implying you reopen a named existing project. Error from the server (`program_name or program_path is required`) is the only signal; the tool schema marks both optional. | `ghidra-headless-mcp` MCP schema + description | Fixed 2026-05-13. Downstream patch `Skills/offensive-macos-tooling-ghidra-headless/open-existing-docstring.patch` extends the tool description at `server.py:283` to state "Requires `program_name` or `program_path` to identify which program inside the project to open; both are listed as optional in the schema but exactly one must be supplied." Auto-applied on next `install-ghidra-host.sh --install` via the patch-stack rsync + dual-probe apply loop (PR #13). PATCHES.md updated. The fully correct upstream fix would be to express the "exactly one of" constraint in the input schema itself (Pydantic discriminator / oneOf), but that's a larger change to the server's tool-spec generator; the docstring patch is the minimum fix that closes the immediate confusion at the catalog-discovery surface. | shipped |
...
| 28 | **app.asar is >100 MB extracted, shouldn't live in `targets/` on disposable lab host.** I extracted locally on the workstation (which is fine per SHAKEDOWN §host-vs-lab-host rules) — but the lab host's `/Users/szeth/Targets/rocket_chat/` already has the asar and unpacked dirs shipped as part of the .app bundle. Workstation extract is redundant with the lab-host state when the operator later attaches an MCP for JS inspection. No tool in this station currently reads `targets/app-asar/` — it's static reference material for Ghidra-style grep. | workstation storage convention + skill docs | Shipped — workstation per electron-pack skill. The Electron surface-pack SKILL.md step 2 invokes `scripts/extract-asar.sh`, and `scripts/` is the workstation-side tree per the "Host vs Lab-host offloading" rule (Ghidra-style grep is fine on the workstation; the lab host doesn't need the extraction for static work). The lab host can extract on demand if dynamic JS inspection is ever needed for a target. No skill text amendment required — the convention is established by the script's location and the step's wording. | shipped |
...
| 35 | **MCP `program.open` silently picks one slice on universal Mach-Os.** Surfaced 2026-05-13 during the SHAKEDOWN #8 probe: `mcp__ghidra-mcp__program_open` against universal `/bin/echo` returned `language_id: x86:LE:64:default` with no warning, no per-slice option. PR #14 (item #29) gave `scripts/ghidra-scan.sh` an auto-slice loop on the SSH path; the MCP path has no equivalent. An agent driving Ghidra entirely via MCP on a universal target never sees the other slice — same blind spot #29 fixed for the SSH wrapper. | `ghidra-headless-mcp` server `program.open` schema + backend (`session_open` in `backend.py`) | Fixed 2026-05-13. Downstream patch `Skills/offensive-macos-tooling-ghidra-headless/refuse-universal-macho.patch` detects Mach-O fat magic (`0xCAFEBABE` / `0xCAFEBABF`) at the earliest unambiguous point in both `session_open` (path-based) and `session_open_bytes` (uploaded blob), raises `GhidraBackendError` with the actionable message "pre-slice with `lipo -thin <arch> <in> <out>` and pass the slice path; or use `scripts/ghidra-scan.sh` which auto-iterates slices." Auto-applied by `install-ghidra-host.sh --install` via PR #13's patch-stack rsync + dual-probe apply loop. PATCHES.md updated. Verified end-to-end on NightBlood: real universal `/bin/echo` refused; both fat32 and fat64 base64-encoded payloads refused; thin Mach-O magic (`0xCFFAEDFE`) passes the gate (negative control). The minimum-viable refuse-loudly variant; auto-iteration would change the tool's return shape (single → list of session_ids), candidate for upstream design discussion. **Action item:** restart any long-lived `ghidra-headless-mcp` PIDs after the next install — editable install rereads `backend.py` only on process start. | shipped |
| 36 | **`program.save` ignores `read_only` session flag.** Surfaced 2026-05-13 during the SHAKEDOWN #8 probe: `mcp__ghidra-mcp__program_save` on a `read_only=true` session returned `saved:true` and successfully wrote the analyzed DB to disk. The flag's documented intent is "do not mutate"; saving the DB is a mutation of the project file. Either `program.save` should refuse on a read-only session (consistent with `function.rename` etc. raising `session ... is read-only`), or the documentation should explicitly carve `program.save` out of the `read_only` contract. Cosmetic in the documented Workflow A (the auto-persistence-via-transaction does the same job), but the inconsistency makes `read_only` a leaky abstraction. | `ghidra-headless-mcp` `backend.py` `program_save` (or whatever it dispatches to) | Not fixed. Decide: (a) make `program_save` raise on read-only (preferred — consistency with other mutators); (b) document that `program_save` is exempt from `read_only` because it persists analysis state Ghidra has already committed inside transactions. | low |
