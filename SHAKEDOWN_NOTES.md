...
| 5 | `ghidra-scan.sh` defaulted to `-Xmx12g` with no regard for VM RAM. On a 4 GB VM this caused 2+ hours of swap thrashing before detection. | `scripts/ghidra-scan.sh` | Default dropped to 10g 2026-05-11 after VM resize to 16 GB. Rule of thumb encoded as comment: `physical_ram_gb - 6`. Still no preflight check. | partial |
| 6 | No PyGhidra preflight check on lab host. `analyzeHeadless` directly errored with "Ghidra was not started with PyGhidra. Python is not available" until we figured out `pyghidra_launcher.py -H` with the MCP venv's Python is the required entry point. | `scripts/install-ghidra-host.sh` / `ghidra-scan.sh` | Documented in `ghidra-scan.sh` header comments. No automated check. | med |
| 7 | No disk-space preflight in `ghidra-scan.sh`. A large analysis that hits ENOSPC mid-run leaves a corrupt `.gpr`. Operator flagged 5 GB free as the manual tripwire. | `scripts/ghidra-scan.sh` | Not implemented. Should `df` before starting and refuse if free space < `max(5_GB, 2x_binary_size)`. | med |
...
| 9 | `scan_tcc_prompt_surface.py`'s `enrich_objc_msgsend` decompiles every `_objc_msgSend` caller before bucketing by selector. On a 177 MB Chromium+Cocoa binary that's hundreds of thousands of decomps; scan phase becomes multi-hour. | `ghidra-scripts/scan_tcc_prompt_surface.py` + `_re_lib.enrich_objc_msgsend` | Not implemented. Should expose `fast_mode=True` or `max_total_callsites` to the scan's top-level `run_string_scan` invocation. Also worth a "target size > N MB → warn" heuristic. | med |
| 10 | No progress beacon during Ghidra auto-analysis. Stdout grows < 1 line/min on large binaries; "making progress" and "deadlocked" look identical without wall-clock + CPU comparison. | Ghidra headless + scan scripts | Not implemented. Two-part: (a) local `ghidra-watch.sh` polling `du` + stdout tail + `ps` every 60s; (b) `_re_lib` stderr heartbeat every ~100 callsites during enrich phases. | high |
| 11 | Ghidra universal-Mach-O slice selection is implicit. No `-loader-arch` flag; caller must `lipo -thin <arch>` first. | `analyzeHeadless` docs + `ghidra-scan.sh` | Documented in `ghidra-scan.sh` header; auto-slice detection not implemented. Could detect universal via `file` and either extract + name consistently or refuse. | low |
| 12 | Ad-hoc ssh polling pattern: repeated `ssh NightBlood '<long command>'` invocations with full handshake each time. Probably minutes of handshake overhead accumulated during PASS-001. | Agent workflow + `~/.ssh/config` | Not implemented. Fix is two lines in `~/.ssh/config`: `ControlMaster auto` + `ControlPersist 10m` on `Host NightBlood`. MCP path already uses persistent SSH — only ad-hoc commands pay the cost. | high |
| 13 | `conv.md` exists in rocket_chat as a per-session handoff artifact but isn't in `.gitignore`. Risk of accidental commit. | rocket_chat template `.gitignore` | Not fixed. Add `conv.md` (and maybe `conv*.md`) to the gitignore template. | low |
| 14 | No VM-sizing recommendation anywhere in skillz. `machines.md` / `offensive-macos-lab-roster` are silent on resources. Operators provision guess-based VMs. | `Skills/offensive-macos-lab-roster/SKILL.md` | Not fixed. Should publish: "for Electron-class binaries, 8 vCPU / 32 GB RAM / 256 GB disk. For Apple framework analysis, 16+ GB RAM, expect multi-hour analysis." | high |
| 15 | No `Host NightBlood` SSH keepalive guidance. A long-running scan over a session-bound SSH MCP transport can drop on idle; `ServerAliveInterval 30` already exists in the MCP wrappers but isn't documented as station policy. | `Skills/offensive-macos-station-topology` | Not fixed. | low |
| 16 | Ghidra's `CreateFunctionCmd` and `VarnodeContext` errors flood the log on complex binaries; drown meaningful INFO lines. Not a bug, but hurts signal-to-noise during polling. | Ghidra | Upstream behavior; no local fix. Can filter `grep -v "^ERROR"` when eyeballing, but the `ghidra-watch.sh` idea above subsumes this. | low |
| 17 | `scan_tcc_prompt_surface.py` `tcc_string` regex generates 24/24 false positives on Electron Framework. `(TCC\|TCCAccessRequest\|kTCCService\|com\.apple\.TCC\|tccd)` with `re.I` matches `TCC` inside base64 root-certificate PEMs. | `ghidra-scripts/scan_tcc_prompt_surface.py` | Not fixed. Drop the lone `TCC` alternative — the four specific forms are precise enough. Same class of false positive likely affects other string rules under re.I. | med |
| 18 | `run_string_scan` does not truncate the `name` column for string-rule anchors. On Electron Framework one PEM cert hit produced an **800 KB** single TSV row. Only the `evidence` column is truncated (120 char). | `ghidra-scripts/_re_lib.py` (run_string_scan) | Not fixed. Truncate `text` to ~160 chars before passing to `writer.add(...)`. | med |
| 19 | `callers_of(external_fn)` yields self-match rows for the import thunk itself. On PASS-001 this produced 2 spurious tier-A anchors whose "caller" is the external symbol `_SecTaskCopyValueForEntitlement` itself. | `ghidra-scripts/_re_lib.py` (callers_of or enrich_callsite_args) | Not fixed. Skip rows where `caller.getName() == external_name` or where callsite address equals the thunk's own entry. | med |
| 20 | String / function index default caps (20k / 50k) are too low for large Apple-framework or Electron binaries. Caps hit on Electron Framework → tier-C coverage bounded. | `ghidra-scripts/_re_lib.py` (`DEFAULT_MAX_STRINGS`, `DEFAULT_MAX_FUNCTIONS`) | Not fixed. Raise defaults to 50k / 100k, or auto-scale with binary size. | med |
| 21 | Ghidra's PyGhidra wraps `println` output in `INFO <script>.py> <line> (GhidraScript)` prefix/suffix. The TSV isn't directly usable from the stdout log without post-processing. | ghidra-scan.sh + lab-host output convention | Not fixed. `ghidra-scan.sh` should emit a clean `.tsv` file alongside the raw `.log` by piping through `sed -E 's/^INFO  [^>]+> //; s/ \(GhidraScript\) +$//'` on exit. | med |
